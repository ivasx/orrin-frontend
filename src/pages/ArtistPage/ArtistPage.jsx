...
export default function ArtistPage() {
    ...
    const menuItems = useMemo(() => {
        ...
        return [
            ...
            {
                id: 'dashboard',
                label: t('artist_dashboard'),
                icon: <LayoutDashboard size={16}/>,
                action: () => navigate(`/artist/${artistSlug}/manage`),
            },
        ];
    }, [canManage, t, artistSlug, navigate]);
    ...
    const heroActions = (
        <>
            ...
            <button className={styles.iconBtn} onClick={handleShare} aria-label={t('share')}>
                <Share2 size={18}/>
            </button>
        </>
    );

    const heroBadge = fullArtist.type === 'group'
    ...
    return (
        <div className={styles.page}>
            <PageHero
                backgroundImage={fullArtist.imageUrl}
                avatar={fullArtist.imageUrl}
                avatarAlt={fullArtist.name}
                avatarShape="circle"
                badge={heroBadge}
                eyebrow={fullArtist.listenersMonthly > 0
            ...
            <MusicSectionWrapper spacing="default">
                <div className={styles.tabContent}>
                    ...
                    {activeTab === 'posts' && (
                        <ArtistPostsTab artistSlug={artistSlug} canPost={canManage}/>
                    )}

                    {activeTab === 'about' && <AboutTab artist={fullArtist}/>}
